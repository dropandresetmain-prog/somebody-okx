...
export function AcquisitionProgress({ view }: { view: ObjectiveWorkspaceView }) {
  return <>{view.external.map(external => <section key={external.providerId} className="mc-acquisition" aria-label={`${external.name} acquisition lifecycle`}>
    <div className="mc-section-heading"><h2>One bounded acquisition</h2><StateLabel value={external.payment.state} /></div><p>{external.name} · {external.resource} · up to ${external.payment.maximumUsd.toFixed(2)} {view.provenance === "frontend_fixture" && <span className="mc-inline-fixture">fixture</span>}</p>
...
export function CompletionSummary({ view }: { view: ObjectiveWorkspaceView }) {
  ...
  return <section className="mc-completion" aria-labelledby="completion-title"><span className="mc-section-label"><Icon name="check" size={15} /> Independent completion verdict{view.provenance === "frontend_fixture" ? " · fixture" : ""}</span><h2 id="completion-title">Required outcome. Verified.</h2><p>{view.completion.summary}</p><ProofLinks refs={view.completion.proofRefs} view={view} /><div className="mc-remaining"><strong>Still pending / outside scope</strong><ul>{view.completion.remaining.map(item => <li key={item}>{item}</li>)}</ul></div></section>;
}
