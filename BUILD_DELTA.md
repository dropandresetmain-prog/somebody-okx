...
| **Minimal workforce kernel (MAKE foundation)** | **Built — first OKX-period product capability** | `dfae75a` | `lib/workforce/` (5 new files), `tests/workforce.test.ts` | `npx tsx --test tests/workforce.test.ts`; `npm test`; `npm run typecheck` | **Focused 11/11 pass. Full suite 89/89 pass (78 inherited + 11 new). Root typecheck clean.** | Scenario-independent foundation the MAKE path and the later Make-vs-Buy policy are built on. |
