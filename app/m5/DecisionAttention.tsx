...
export function Decisions({ view }: { view: ObjectiveWorkspaceView }) {
  ...
  return <section className="mc-decisions" aria-labelledby="decision-title"><div className="mc-section-heading"><h2 id="decision-title">Somebody’s call</h2><StateLabel value={current.authorization} /></div>
    ...
    {selected && <details className="mc-options"><summary>Inspect recommended option · {selected.label}</summary><OptionFacts option={selected} /></details>}
    {alternatives.length > 0 && <details className="mc-options"><summary>{alternatives.length} alternatives considered · MAKE / BUY / HYBRID</summary>{alternatives.map(option => <OptionFacts key={option.optionId} option={option} />)}</details>}
...
export function NeedsYou({ view, onApprove }: { view: ObjectiveWorkspaceView; onApprove?: () => void }) {
  return <section className="mc-attention" aria-labelledby="attention-title">
    ...
    {view.attention.length ? view.attention.map(item => <article key={item.id} className={`mc-attention-item mc-attention-${item.kind}`}>
      ...
      {item.maximumUsd !== null && <div className="mc-spend"><strong>${item.maximumUsd.toFixed(2)}</strong><span>maximum · one acquisition<br />illustrative fixture amount</span></div>}
      ...
      <small>{item.kind === "approval" ? "Fixture approval only. No money moves." : "Paused safely. No automatic retries or invented authority."}</small>
